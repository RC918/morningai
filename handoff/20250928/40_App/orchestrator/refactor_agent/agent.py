...
class RefactorAgent:
    ...
    def _generate_branch_name(self) -> str:
        ...
        return f"refactor/ts-fixes-{timestamp}"

    def _generate_pr_title(self, errors_fixed: int) -> str:
        """
        ...
        return f"fix(ts): Automated TS strict mode fixes ({errors_fixed} errors) - {date_str}"
...
"""
    ...
    def create_pr(
        ...
        commit_message = self._generate_pr_title(len(completed_tasks))
        if not self._commit_fixes(tasks, commit_message):
        ...
        try:
            title = self._generate_pr_title(len(completed_tasks))
