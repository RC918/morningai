...
"""
    ...
    def _check_existing_refactor_pr(self):
        """
        Check if there's an existing open refactor PR.
        ...
        try:
            open_prs = repo.get_pulls(state='open', base='main')
            for pr in open_prs:
                if pr.title.startswith("refactor(typescript):") or \
                   "refactor" in [label.name.lower() for label in pr.labels]:
                    if "automated" in [label.name.lower() for label in pr.labels]:
                        return pr
            return None
        except Exception as e:
            logger.warning(
    ...
    def create_pr(
        ...
        # Check for existing open refactor PRs to avoid duplicates
        ...
        if not self._commit_fixes(tasks, title):
            if not self._checkout_main():
                logger.error("[RefactorAgent] Failed to checkout main after commit failure")
            return (None, None)

        if not self._push_branch(branch_name):
            if not self._checkout_main():
                logger.error("[RefactorAgent] Failed to checkout main after push failure")
            return (None, None)
        ...
        if repo is None:
            ...
            if not self._checkout_main():
                logger.error("[RefactorAgent] Failed to checkout main after GitHub failure")
            return (None, None)
        ...
        try:
            ...
            if labels:
                try:
                    ...
                    logger.warning(
                        "[RefactorAgent] Failed to add labels: %s", e
                    )

            logger.info(
                "[RefactorAgent] Created PR #%d: %s",
                pr.number, pr.html_url
            )

            if not self._checkout_main():
                logger.warning("[RefactorAgent] Failed to checkout main after PR creation")
            ...
            if not self._checkout_main():
                logger.error("[RefactorAgent] Failed to checkout main after PR creation failure")
            return (None, None)
