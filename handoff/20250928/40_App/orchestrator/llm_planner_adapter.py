...
class LLMPlannerAdapter:
    ...
    def generate_plan(
        ...
        """
        if not self.client:
            logger.warning(f"[LLM Planner] OpenAI client not available, using static plan")
        ...
        try:
            ...
            if self._validate_plan(plan_data["plan"]):
                ...
                return {
                    "plan": [step["step"] for step in plan_data["plan"]],
                ...
                logger.warning(f"[LLM Planner] Generated invalid plan, falling back to static")
                return self._get_static_plan(task_type)
...
Generate a 3-7 step plan to accomplish this goal."""
    ...
    def _get_code_context(self, repo: str, goal: str, max_tokens: int = 2000) -> str:
        ...
        context += f"Goal: {goal}\n"
        context += "\nNote: Full code context extraction will be implemented in ContextManager"
