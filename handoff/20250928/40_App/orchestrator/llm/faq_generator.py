...
from .client import get_client_for_component
...
def generate_faq_content(
    ...
    """
    Generate FAQ content using LLMClient abstraction.

    Uses get_client_for_component() to enable A/B testing and
    dynamic provider switching based on experiments.
    ...
    """
    client = get_client_for_component(
        component="faq_generator",
        trace_id=trace_id,
        model=model
    )

    logger.info("Generating FAQ content", extra={
...
- Task: {question}
- Trace ID: `{trace_id}`
- Generated by: MorningAI Orchestrator using {response.model}
- Provider: {response.provider}
- Repository: {repo}
"""

        tokens_used = response.usage.get("total_tokens") if response.usage else None
        logger.info("FAQ content generated successfully", extra={
            "operation": "generate_faq",
            "trace_id": trace_id,
...
def get_cached_or_generate(
    ...
    if use_cache:
        ...
        if cache_key in _faq_cache:
            logger.info("Returning cached FAQ content", extra={
            ...
            return cached.replace(
                "Trace ID: `{trace_id}`", f"Trace ID: `{trace_id}`"
            )
