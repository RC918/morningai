"""
Wrapper tests for VMProvisioner from meta_agent module.

This file imports and re-exports tests from meta_agent/tests/test_vm_provisioner.py
to ensure they are discovered by pytest when running from the tests/ directory.
...
class TestDockerVMProvider:
    ...
    async def test_create_no_docker(self):
        ...
        with patch.object(
            provider, "_get_client", side_effect=Exception("Docker not available")
        ):
