...
class TestExecuteDocsSafety:
    ...
    def test_execute_uses_docs_rate_limit(
        ...
        # Verify rate limit was called with docs max (default 3)
    ...
    def test_execute_rate_limited_returns_early(
        ...
        # Issue #2969: Rate limit check now happens BEFORE create_branch/commit_file
        # So no GitHub side-effects should occur when rate limited (no orphan branches)
    ...
    @patch('graph.create_branch')
    @patch('graph.get_repo')
    def test_rate_limit_not_consumed_when_value_gate_blocks(
        self, mock_get_repo, mock_create_branch, mock_generate_faq,
        mock_value_gate, mock_rate_limit
    ):
        """Test rate limit counter NOT incremented when value gate blocks (Issue #2969)"""
        mock_repo = Mock()
        mock_get_repo.return_value = mock_repo
        mock_create_branch.return_value = "orchestrator/1234567-faq-update"
        mock_generate_faq.return_value = "# FAQ Content\n\nTest content with enough length to pass validation.\n\nGenerated by MorningAI\ntrace-id: test-123"
        
        # Value gate blocks PR creation
        mock_value_gate_result = Mock()
        mock_value_gate_result.should_create_pr = False
        mock_value_gate_result.score = 0.1
        mock_value_gate_result.primary_change_type = Mock(value="trivial")
        mock_value_gate_result.downgrade_reason = "Low value change"
        mock_value_gate.return_value = mock_value_gate_result
        
        pr_url, state, trace_id = execute("Test FAQ", "owner/repo", trace_id="test-123")
        
        # Issue #2969: Rate limit should NOT be called when value gate blocks
        # because rate limit check is now AFTER value gate
        assert state == "value_gate_blocked"
        mock_rate_limit.assert_not_called()
        # Also verify no GitHub side-effects (no orphan branches)
        mock_create_branch.assert_not_called()
    
    @patch('graph.check_pr_rate_limit')
    @patch('graph.check_pr_deduplication')
    @patch('graph.check_value_gate')
    @patch('graph.generate_faq_content')
    @patch('graph.create_branch')
    @patch('graph.get_repo')
    def test_rate_limit_not_consumed_when_dedup_blocks(
        self, mock_get_repo, mock_create_branch, mock_generate_faq,
        mock_value_gate, mock_dedup, mock_rate_limit
    ):
        """Test rate limit counter NOT incremented when dedup blocks (Issue #2969)"""
        mock_repo = Mock()
        mock_get_repo.return_value = mock_repo
        mock_generate_faq.return_value = "# FAQ Content\n\nTest content with enough length to pass validation.\n\nGenerated by MorningAI\ntrace-id: test-123"
        ...
        mock_value_gate_result.score = 0.8
        mock_value_gate_result.primary_change_type = Mock(value="documentation")
        mock_value_gate.return_value = mock_value_gate_result

        # Dedup blocks PR creation
        mock_dedup_result = Mock()
        mock_dedup_result.is_duplicate = True
        mock_dedup_result.should_create_pr = False
        mock_dedup_result.duplicate_type = "exact_match"
        mock_dedup_result.similarity_score = 1.0
        mock_dedup.return_value = mock_dedup_result
        
        pr_url, state, trace_id = execute("Test FAQ", "owner/repo", trace_id="test-123")
        
        # Issue #2969: Rate limit should NOT be called when dedup blocks
        assert state == "duplicate_blocked"
        mock_rate_limit.assert_not_called()
        # Also verify no GitHub side-effects (no orphan branches)
        mock_create_branch.assert_not_called()
    ...
    @patch('graph.settings')
    @patch('graph.check_pr_rate_limit')
    @patch('graph.acquire_pr_lease')
    @patch('graph.generate_faq_content')
    @patch('graph.get_repo')
    def test_rate_limit_not_consumed_when_lease_blocks(
        self, mock_get_repo, mock_generate_faq, mock_acquire_lease,
        mock_rate_limit, mock_settings
    ):
        """Test rate limit counter NOT incremented when lease acquisition fails (Issue #2969)"""
        mock_settings.enable_pr_deduplication = True
        mock_settings.pr_dedup_dry_run = False
        ...
        # Lease acquisition fails (another worker holds the lease)
        mock_lease_result = Mock()
        mock_lease_result.acquired = False
        mock_lease_result.holder = "worker-other"
        mock_lease_result.existing_pr_url = None
        mock_lease_result.reason = "Lease held by another worker"
        mock_acquire_lease.return_value = mock_lease_result
        
        pr_url, state, trace_id = execute("Test FAQ", "owner/repo", trace_id="test-123")
        
        # Issue #2969: Rate limit should NOT be called when lease acquisition fails
        assert state == "lease_blocked"
        mock_rate_limit.assert_not_called()
    
    @patch('graph.release_pr_lease')
    @patch('graph.settings')
    @patch('graph.check_pr_rate_limit')
    @patch('graph.check_pr_deduplication')
    @patch('graph.check_value_gate')
    ...
    @patch('graph.generate_faq_content')
    @patch('graph.get_repo')
    def test_rate_limited_releases_lease_when_acquired(
        self, mock_get_repo, mock_generate_faq, mock_acquire_lease,
        mock_value_gate, mock_dedup, mock_rate_limit, mock_settings, mock_release_lease
    ):
        """Test rate-limited path releases lease when lease was acquired (Issue #2969)"""
        mock_settings.enable_pr_deduplication = True
        mock_settings.pr_dedup_dry_run = False
        mock_settings.orchestrator_dry_run = False
        mock_settings.orchestrator_test_mode = True
        ...
        mock_lease_result = Mock()
        mock_lease_result.acquired = True
        mock_acquire_lease.return_value = mock_lease_result
        ...
        mock_dedup_result.should_create_pr = True
        mock_dedup_result.duplicate_type = None
        mock_dedup_result.similarity_score = 0.0
        mock_dedup.return_value = mock_dedup_result
        
        # Rate limited - already created max PRs
        mock_rate_limit.return_value = (False, 3)
        
        pr_url, state, trace_id = execute("Test FAQ", "owner/repo", trace_id="test-123")
        
        # Issue #2969: When rate limited after acquiring lease, lease should be released
        assert state == "rate_limited"
        mock_release_lease.assert_called_once()  # Lease must be released to prevent deadlock
