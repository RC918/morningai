...
class PreAuthTokenManager:
    ...
    def __init__(self):
        ...
        self.jwt_secret = os.environ.get('JWT_SECRET_KEY', 'test-secret-key-for-testing')
    
    def generate_token(
        self,
        user_id: str,
        email: str,
        scope: Literal['enroll', 'challenge']
    ) -> str:
        """
        ...
        logger.info(f"Pre-auth token generated for user {user_id}, scope: {scope}, jti: {jti}")
    ...
    def verify_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not token_data:
                logger.warning(f"Token jti {jti} not found in Redis (expired or invalid)")
                return None
            ...
            if attempts >= MAX_ATTEMPTS_PER_TOKEN:
                logger.warning(f"Token jti {jti} exceeded max attempts ({MAX_ATTEMPTS_PER_TOKEN})")
                return None
