...
async def api_review_security_event(event_data):
    ...
    if isinstance(event_data, dict):
        ...
        human_review = event_data.get('requires_human_review', risk_score > 0.7)
    else:
        source_ip = getattr(event_data, 'source_ip', '127.0.0.1')
        user_id = getattr(event_data, 'user_id', None) or getattr(
            event_data, 'user', None
        )
        human_review = getattr(
            event_data, 'requires_human_review', risk_score > 0.7
        )
...
async def api_submit_hitl_review(request_data: Dict[str, Any]):
    ...
    if isinstance(event_data, dict):
        hitl_source_ip = event_data.get('source_ip', '127.0.0.1')
    else:
        hitl_source_ip = getattr(event_data, 'source_ip', '127.0.0.1')
