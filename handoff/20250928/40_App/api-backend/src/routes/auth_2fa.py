...
from ..services.auth_service import generate_access_token, generate_refresh_token, set_auth_cookies, get_user_by_id
...
def enroll_2fa():
    ...
    try:
        ...
        if existing_2fa.data and existing_2fa.data[0].get('enabled') and existing_2fa.data[0].get('verified_at'):
...
def verify_enroll_2fa():
    ...
    try:
        ...
        if not user_2fa.data:
            return jsonify({'error': '2FA enrollment not started. Call /enroll first.'}), 400
        ...
        if not totp_manager.verify_totp(secret, code):
            ...
            logger.warning(f"Invalid TOTP code for user {user_id} during enrollment (attempt {attempts})")
...
def challenge_2fa():
    ...
    try:
        ...
        if backup_code:
            ...
            backup_codes_data = supabase.table('totp_backup_codes').select('*').eq('user_id', user_id).eq('used', False).execute()
            
            if not backup_codes_data.data:
                return jsonify({'error': 'No valid backup codes remaining'}), 401
            ...
            if not valid_code_id:
                ...
                logger.warning(f"Invalid backup code for user {user_id} during login (attempt {attempts})")
            ...
            remaining_codes = supabase.table('totp_backup_codes').select('id').eq('user_id', user_id).eq('used', False).execute()
            backup_codes_remaining = len(remaining_codes.data) if remaining_codes.data else 0
            
            logger.info(f"User {user_id} logged in with backup code, {backup_codes_remaining} codes remaining")
            ...
            if not totp_manager.verify_totp(secret, code):
                ...
                logger.warning(f"Invalid TOTP code for user {user_id} during login (attempt {attempts})")
                return jsonify({'error': 'Invalid TOTP code'}), 401
        ...
        if remember_device:
            try:
                ...
                device_fingerprint = generate_device_fingerprint(user_agent, remote_addr)
