...
def _get_orchestrator_metrics():
    ...
    try:
        ...
        if orchestrator_path not in sys.path:
            sys.path.insert(0, os.path.abspath(orchestrator_path))
...
def _collect_api_metrics(window_minutes: int = 15) -> Dict[str, Any]:
    ...
    try:
        ...
        failed_keys = redis_client.keys('rq:failed:*')
...
def _collect_rate_limit_metrics(window_minutes: int = 15) -> Dict[str, Any]:
    ...
    try:
        ...
        active_limits = len(rate_limit_keys) if rate_limit_keys else 0
...
def _collect_session_command_metrics(window_minutes: int = 15) -> Dict[str, Any]:
    ...
    try:
        session_keys = redis_client.keys('session:*:data')
        command_keys = redis_client.keys('session:*:commands')

        active_sessions = len(session_keys) if session_keys else 0
        pending_commands = 0

        for key in (command_keys or []):
...
def get_metrics():
    ...
    window_minutes = int(request.args.get('window', 15))
