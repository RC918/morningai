...
def test_redis_unavailable_returns_503(app):
    ...
    with app.test_client() as client:
        with patch('src.routes.agent.get_agent_redis_client') as mock_get_client:
            ...
            mock_client.hset.side_effect = RedisConnectionError("Connection refused")
...
def test_redis_retry_with_sentry_trace(app):
    ...
    with app.test_client() as client:
        with patch('src.routes.agent.sentry_sdk') as mock_sentry:
            with patch('src.routes.agent.get_agent_redis_client') as mock_get_client:
                mock_client = MagicMock()
                mock_client.hset.side_effect = RedisConnectionError("Connection timeout")
                mock_get_client.return_value = mock_client
...
def test_redis_connection_with_retry_succeeds(app):
    ...
    with app.test_client() as client:
        with patch('src.routes.agent.get_agent_redis_client') as mock_get_client:
            mock_client = MagicMock()
            mock_client.hset.side_effect = intermittent_failure
