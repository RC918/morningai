...
class MonitoringSystem:
    ...
    def run_health_checks(self) -> List[HealthCheckResult]:
        ...
        # 檢查 /openapi.json
        results.append(self.check_endpoint('/openapi.json'))
        
        # 檢查 /auth/register (dry-run)
        results.append(self.check_endpoint('/auth/register', method='POST', 
                                         json_data={}, dry_run=True))
        
        # 檢查 /referral/stats (需要認證)
        if self.auth_token:
            headers = {"Authorization": f"Bearer {self.auth_token}"}
            results.append(self.check_endpoint('/referral/stats', headers=headers))
        else:
            logger.warning("No auth token provided, skipping /referral/stats check")
    ...
    def check_alerts(self) -> List[Dict]:
        ...
        # 檢查連續 5xx 錯誤
        recent_results = list(self.health_results)[-10:]  # 檢查最近 10 次結果
        consecutive_5xx = 0
        ...
        if consecutive_5xx >= self.alert_config.consecutive_5xx_critical:
            alerts.append({
                'level': 'CRITICAL',
                'type': 'CONSECUTIVE_5XX',
                'message': f'連續 {consecutive_5xx} 次 5xx 錯誤',
                'value': consecutive_5xx,
                'threshold': self.alert_config.consecutive_5xx_critical
            })
        
        # 檢查連續健康檢查失敗
        health_checks = [r for r in recent_results if r.endpoint in ['/health', '/healthz']]
        consecutive_health_fail = 0
        for result in reversed(health_checks):
            if not result.success or result.status_code != 200:
                consecutive_health_fail += 1
            else:
                break
        
        if consecutive_health_fail >= self.alert_config.consecutive_health_fail_critical:
            alerts.append({
                'level': 'CRITICAL',
                'type': 'HEALTH_CHECK_FAIL',
                'message': f'連續 {consecutive_health_fail} 次健康檢查失敗',
                'value': consecutive_health_fail,
                'threshold': self.alert_config.consecutive_health_fail_critical
            })
    ...
    def generate_daily_report(self) -> Dict:
        ...
        peak_5xx_hour = max(hour_5xx_counts.items(), key=lambda x: x[1])[0] if hour_5xx_counts else None
        ...
        return {
            ...
            'peak_5xx_hour': f"{peak_5xx_hour}:00-{peak_5xx_hour+1}:00" if peak_5xx_hour is not None else "無",
...
def main():
    """主函數"""
    # 從環境變數讀取配置
    base_url = os.getenv('MONITOR_BASE_URL', 'http://localhost:8000')
