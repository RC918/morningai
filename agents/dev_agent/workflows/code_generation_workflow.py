...
class CodeGenerationWorkflow:
    ...
    async def analyze_context(self, state: CodeGenState) -> CodeGenState:
        ...
        Issue #3628: Canonicalize extracted file paths using changed_files.
        ...
        try:
            ...
            if target_files and changed_files:
                ...
                for extracted_path in target_files:
                    canonical = self._canonicalize_path(extracted_path, changed_files)
                    if canonical and canonical not in canonicalized:
                        canonicalized.append(canonical)
                    elif extracted_path not in canonicalized:
                        # Keep original if no canonical match found
...
"""
    ...
    def _canonicalize_path(
        ...
        Issue #3628: When the LLM extracts just a filename (e.g., 'probe0_lint_error.py')
        ...
        Returns:
            Full path from changed_files if unambiguous match found, None otherwise
        """
        ...
        if '/' in extracted_path:
            ...
            # No exact match, return None to keep original
            return None
        ...
        if len(matches) == 1:
            ...
            # Fail closed: don't guess, keep original and let security validation decide
            logger.warning(
                f"[Stage 2] Ambiguous basename '{extracted_path}' matches {len(matches)} files: "
                f"{matches}. Keeping original path."
            )
            return None
        else:
            # No match found
