...
"""
    ...
    def _is_safe_file_path(self, file_path: str, task_metadata: dict = None) -> bool:
        ...
        try:
            ...
            # If task_metadata specifies allowed_directories, enforce them
            if task_metadata and 'allowed_directories' in task_metadata:
                allowed_dirs = task_metadata['allowed_directories']
                if allowed_dirs:
                    ...
                    # Check if file matches any allowed directory or file
                    is_allowed = False
                    for allowed in allowed_dirs:
                        # Normalize allowed path
                        allowed_normalized = os.path.normpath(allowed)
                        
                        # Check if it's an exact file match (e.g., "README.md")
                        if rel_path == allowed_normalized:
                            is_allowed = True
                            break
                        
                        # Check if it's under an allowed directory (e.g., "docs/")
                        if allowed_normalized.endswith('/'):
                            # Directory prefix match
                            if rel_path.startswith(allowed_normalized) or rel_path.startswith(allowed_normalized.rstrip('/')):
                                is_allowed = True
                                break
                        else:
                            # Could be a file or directory without trailing slash
                            # Check both exact match and prefix match
                            if rel_path == allowed_normalized or rel_path.startswith(allowed_normalized + '/'):
                                is_allowed = True
                                break
                    
                    if not is_allowed:
                        logger.warning(
                            f"Blocked path outside allowed directories: {file_path} -> {rel_path}. "
