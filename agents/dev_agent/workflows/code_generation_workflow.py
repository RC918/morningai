...
"""
    ...
    def _normalize_file_path(self, raw_path: str) -> Optional[str]:
        ...
        # Match pattern: path:number or path:number:number
        line_col_match = re.match(r'^(.+?\.(py|js|ts|jsx|tsx|md)):\d+(?::\d+)?$', path)
        if line_col_match:
        ...
        # Validate it looks like a real path (has at least one path separator or is a simple filename)
        if not path or path.startswith('.'):
            logger.debug(f"Rejected invalid path: {path}")
            return None
