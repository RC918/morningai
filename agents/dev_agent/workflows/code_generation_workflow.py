    ...
    def _is_safe_file_path(self, file_path: str, task_metadata: dict = None) -> bool:
        ...
        try:
            ...
            if task_metadata and ('allowed_directories' in task_metadata or 'allowed_files' in task_metadata):
                ...
                # Check if file matches any allowed file (exact match)
                is_allowed = False
                for allowed_file in allowed_files:
                    allowed_file_path = os.path.normpath(allowed_file)
                    if rel_path == allowed_file_path:
                        is_allowed = True
                        break
                
                # Check if file is under any allowed directory (prefix match)
                if not is_allowed:
                    for allowed_dir in allowed_dirs:
                        allowed_dir_path = os.path.normpath(allowed_dir)
                        
                        # Directory containment check for paths like "docs/"
                        # os.path.join ensures correct separator and handles trailing slashes
                        dir_prefix = os.path.join(allowed_dir_path, '')
