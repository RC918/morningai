...
PROTECTED_PATTERNS = [
    r'^release/.*',
    r'^hotfix/.*',  # Keep recent hotfixes, but allow old ones to be cleaned
    r'^gh-pages.*',
]
...
def get_remote_branches() -> List[str]:
    ...
    for line in output.split('\n'):
        ...
        if branch and not branch.startswith('origin/HEAD'):
            # Remove 'origin/' prefix for easier handling
...
def get_branch_last_commit_date(branch: str) -> datetime:
    ...
    if not success:
        return datetime.now()  # Return now if we can't get date (won't be deleted)

    # Parse date like "2025-09-28 12:34:56 +0000"
    try:
        date_str = output.split()[0]  # Just get YYYY-MM-DD
...
def matches_pattern(branch: str, pattern: str) -> bool:
    """Check if branch matches the given glob-like pattern."""
    if not pattern:
        return True
...
def get_branches_with_open_prs() -> Set[str]:
    """Get branches that have open PRs (should not be deleted)."""
...
def delete_branch(branch: str) -> bool:
    ...
    success, output = run_git_command(['git', 'push', 'origin', '--delete', branch])
    return success
...
def print_summary(
    ...
    print(f"Total branches analyzed: {len(to_delete) + len(to_keep)}")
    ...
    for branch, reason, age in sorted(to_delete, key=lambda x: -x[2])[:50]:
...
def main():
    ...
    if not args.skip_pr_check:
        ...
        open_pr_branches = get_branches_with_open_prs()
